...
## Time Series Analysis Using Bayesian Structural Time Series 
...
Here we have fit a BSTS model and plot each of the components that we discussed separated out. In red, we have the true values of the observations. The yellow curve is the autoregressive component (i.e. how much ridership in day $t$ depends on ridership in day $t-1$). The green and blue curves are the seasonal components, with green tracking daily "seasonality" (i.e. the day of the week effect) and blue tracking seasonality in the literal sense: differences between the actual seasons. Finally, in purple is the underlying trend.

We can use this BSTS model to predict out future ridership. The predictions from our model look like this:
...
df %>%
  ...
  ggplot(aes(date, numrides / 10**3, color = type))+
  geom_line()+
  geom_ribbon(aes(ymax = upper / 10**3, ymin = lower / 10**3), alpha = .3, fill = 'gray', color = 'darkgray')+
  theme_fivethirtyeight()+
  theme(legend.position = 'bottom', legend.direction = 'horizontal', axis.title.y = element_text())+
  labs(y = "Rides (Thousands)", caption = 'Gray ribbon denotes 95% credible interval for the mean prediction', color = 'Type')
